...
def hash(data):
    ...
    for i in range(0, len(data), 4):
        ...
        state[7] ^= block[3]
        print(f"Iteration {i}:")
        print("After XOR with block:", state)
        print("matrix bit form:")
        for x in state:
            print(f"{x:08b}")
        state = permute(state)
        ...
        for idx in range(8):
            x = state[idx]
            s = SBOX[x]
            for x_new, s_value in enumerate(SBOX):
                if s_value == s and x_new != x:
                    check = (x_new ^ x) % (2**4) # we can't substitute the very first 4 states
                    if check == 0:
                        print(f"we have won: idx={idx}, x={x}, s={s}, x_new={x_new}")
                        print(f"bit form: x={x:08b}, x_new={x_new:08b}")

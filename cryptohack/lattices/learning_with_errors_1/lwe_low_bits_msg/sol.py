...
q = 0x10001 #2^16+1 ciphertext modulus, coprime with p
...
# Compute x=b−⟨A,S⟩ mod q and then interpret x as an integer (not modulo q)
# Compute m=x mod p, where the division and rounding happens over the integers
# return m
...
def lwe_decrypt(A, b, S, q):
    ...
    x = (b - inner_product) % q
    # Interpret x as an integer between -q/2 and q/2
    if x > q // 2:
        x -= q
    # Compute m = x mod p
    m = x % p
